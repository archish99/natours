...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).select('+password'); // Selecting password explicitly
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      // Put the currentUser on the locals in response, pug templates have access to locals object
      res.locals.user = currentUser;
      return next();
    } catch (err) {
      return next();
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    // roles is an array of the values passed, here -> ['admin', 'lead-guide']
